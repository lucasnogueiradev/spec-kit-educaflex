...
Diferente de templates passivos, esta CLI atua como um juiz mecânico: ela lê o seu planejamento, converte em tarefas, gera os esqueletos de testes e **audita mecanicamente** se a implementação (feita por você ou uma IA externa) cumpriu com o prometido.

## 🚀 Como funciona o Pipeline (`educaflex run`)

Ao rodar o comando único, a CLI executa 4 passos de forma autônoma:
...
4. **Auditoria de Intenção**: Cruza os testes rodando com a lista de Critérios de Aceitação da spec original. Se um critério faltar ou falhar, a pipeline quebra, devolvendo o controle para que você ou sua IA externa implementem o código.
