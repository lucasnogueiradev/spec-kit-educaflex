...
4. **Implementação (IA)**: Aciona o agente de IA simulado/integrado para programar o código-fonte baseado nas tarefas.
5. **Rodar Testes**: Executa o `vitest` em background e extrai o relatório estruturado (JSON).
6. **Auditoria de Intenção**: Cruza os testes aprovados pelo `vitest` com a lista de Critérios de Aceitação da spec original. Se um critério faltar ou falhar, a pipeline quebra.
...
✅ [AC_APROVADO] AC-001: Login com credenciais válidas
✅ [AC_APROVADO] AC-002: Login com credenciais inválidas
==============================
